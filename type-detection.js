...
// that there's no syntax for you to explicitly specify types. If you come from
// a statically typed language, this may sound blasphemous, but try to think
// about it as one less thing to worry about. While this is generally true,
// there are occasionally cases, where you have to know what type some input is
// in order to take appropriate action, and that's where this module's content
// comes in handy.

// Before we go into type detection itself, let's talk about types in
// JavaScript.
...
// In both the browsers and on NodeJS, you will notice something strange. When
// you call `typeof()` on `null` you will get "object" instead of "null". This
// is generally a big omission in the JavaScript specification, and `null` is
// the only primitive value whose type you cannot test with `typeof`. Luckily,
// `null` is the only value of `null` type, so you can simply test for equality:
...
// Functions are the only non-primitive type that you can detect using
// `typeof()` and it returns "function" as expected.
...
// We mentioned that `instanceof` operator is one of the ways to test the type
// of built-in non-primitives. Another way is a bit hackish, but you will see
// that it is surprisingly useful. Every plain object has a `toString()`
// function. For example:

console.log('({n: 12}).toString():', ({n: 12}).toString()) // [object Object]

// This function, when applied to objects other than the plain objects, gives
// us information about their type. Since this function is a property on the
// objects themselves, we can access it as `Object.prototype.toString()` (see
// the `proto.js` module for more information on how this works).
...
// Interesting thing about this approach is that it also works on pritimitive
// types.
...
// Not only does it work on primitive types, but it is also able to
// differentiate between `null` and other objects, so it's a double-win. Since
// the whole business of using `Object.prototype.toString.call()` is a bit too
// much, we'll wrap it in a function and also clean up the output a little.
...
// NOTE: The above example is actuallly a very good case where you cannot avoid
// type detection because you *can* add 1 to undefined, in which case you would
// get  a `NaN` as a result, and your code will not throw an exception. It makes
// sense mathematically in a way, but hardly useful in real life.
...
// Duck typing is the opposite of type detection. Instead of checking whether
// the thing is a duck, we check that it quacks, or has feathers, whatever
// aspect are interested in. For instance, if your function takes an object, and
// you need to invoke `foo()` on the object, you may test that it is an object,
// then test that the `foo` property is a function, and then you finally call
// it. Or, with duck typing, you could simply call it and pray for the best.
// Although the latter approach sounds like a better fit for religious people,
// it works surprisingly well in many many cases. Still, duck typing in
// JavaScript does not work as well as it does in, say, Python. The main reason
// for this is that trying to access a missing property on an object evaluates
// to `undefined` *without* throwing an exception. Here's an example:
